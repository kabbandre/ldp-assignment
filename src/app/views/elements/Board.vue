<template>
  <div class="board-container">
    <div class="board">
      ...
      <div v-if="false" class="clear-button">
          ...
          <Draggable v-for="(list, listIndex) in lists" :key="list.id">
            <section class="list-container" ref="list" :data-id="list.id">
              ...
              <div class="list-header">
                <span v-if="false" class="list-drag-handle">&#x2630;</span>
                {{ list.title }}
              </div>
              ...
              >
                <Draggable v-for="item in list.items" :key="item && item.id">
                  <Card :item="item" :list="list" @edit="editItem"/>
                </Draggable>
  ...
  methods: {
    ...
    onAddFullItem (item) {
      item.id
        ...
        : this.addItem(this.activeListId, item.title, item.description, item.date, item.priority)
      this.hideModal()
    },

    addItem (listId, title, description, date, priority) {
      this.$store.commit('addItem', { listId, title, description, date, priority })
    },
